...
export class AIService {
  ...
  constructor() {
    ...
    this.defaultModel =
      process.env.LLM_MODEL || "meta-llama/llama-3-70b-instruct:free";
...
Hãy nhẹ nhàng thử thách các giả định của họ, sử dụng danh xưng "đồng chí" hoặc "bạn" một cách lịch sự, tri thức và mang tính học thuật. Tránh viết các khối văn bản quá lớn. Hãy luôn tranh luận bằng Tiếng Việt (Vietnamese) và đưa ra đúng một câu hỏi tập trung ở cuối phản hồi.`;
      ...
      return (
        ...
        "Chúng ta hãy suy ngẫm sâu sắc hơn về vấn đề này."
      );
    } catch (error) {
      this.logger.error(
        `Socratic debate AI completion failed: ${error.message}`,
      );
      return this.getMockDebateResponse(userQuery);
    }
  }
  ...
  "chapters": [
    {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Extract course architecture for: "${courseTitle}". Document slice:\n\n${documentContent.substring(0, 12000)}`,
          },
        ],
...
[
  ...
  ...
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Write a conversation script on: "${conceptTitle}". Context: ${summary}`,
          },
        ],
        response_format: { type: "json_object" },
        temperature: 0.6,
      ...
      const responseText =
        completion.choices[0]?.message?.content || '{"script": []}';
      const parsed = JSON.parse(responseText);
      return (
        parsed.script ||
        parsed.dialogue ||
        parsed ||
        this.getMockPodcastScript()
      );
    } catch (error) {
      this.logger.error(
        `Podcast script AI generation failed: ${error.message}`,
      );
      return this.getMockPodcastScript();
    }
  }
  ...
  private getMockPodcastScript(): { speaker: string; text: string }[] {
    return [
      {
        speaker: "Host",
        text: "Chào mừng các đồng chí quay trở lại với PhiloMind Podcasts. Hôm nay, chúng ta sẽ đi sâu vào nghịch lý nổi tiếng nhất của triết gia Jean-Paul Sartre.",
      },
      {
        speaker: "Host",
        text: "Khi Sartre nói con người 'bị kết án phải tự do', nghe có vẻ thật nặng nề, tựa như một bản án tù. Ý của ông ấy là gì thưa đồng chí khách mời?",
      },
      {
        speaker: "Guest",
        text: "Quả thực là một khái niệm rất sâu sắc. Sự kết án ở đây chính là sự thật không thể trốn chạy: đồng chí không thể lựa chọn việc không lựa chọn.",
      },
      {
        speaker: "Guest",
        text: "Ngay cả khi đồng chí chọn im lặng, chọn rút lui hay để người khác quyết định thay mình, thì đó cũng chính là một lựa chọn chủ động mà đồng chí đã đưa ra.",
      },
      {
        speaker: "Host",
        text: "Như vậy là hoàn toàn không có sự giải thoát nào khỏi trách nhiệm của bản thân sao? Không có tấm lưới an toàn đạo đức nào ư?",
      },
      {
        speaker: "Guest",
        text: "Hoàn toàn không có. Không có giá trị ngoại cảnh nào để đổ lỗi cả. Chúng ta chính là những tác giả duy nhất viết nên số phận của cuộc đời mình.",
      },
    ];
  }
          ...
          nodes: [
            {
              ...
              summary:
                "Albert Camus lập luận rằng con người luôn có khao khát nội tại tìm kiếm ý nghĩa, trật tự và mục đích trong cuộc sống. Tuy nhiên, vũ trụ lại lạnh lẽo, im lặng và vốn dĩ không có bất kỳ ý nghĩa khách quan nào. Sự xung đột giữa khát vọng ý nghĩa của con người và sự im lặng của vũ trụ chính là 'Sự phi lý'.",
              originalText:
                "Sự phi lý được sinh ra từ sự đối đầu giữa nhu cầu của con người và sự im lặng vô lý của thế giới. - Albert Camus",
              quickTake:
                "Sự đối đầu trực diện giữa khát vọng ý nghĩa của con người và sự im lặng vô tận của vũ trụ.",
              difficulty: "Medium",
              ...
              flashcards: [
                {
                  tag: "Thuyết Phi lý",
                  question:
                    "Theo Camus, con người nên đối phó với 'Sự phi lý' như thế nào?",
                  answer:
                    "Camus từ chối tự sát vật lý và tự sát triết học (tin vào giáo điều). Ông đề xuất sự nổi loạn: chấp nhận sự phi lý và sống một cách đầy kiêu hãnh, tự do và đam mê.",
                },
              ],
            },
            {
              ...
              summary:
                "Trong hữu thể học hiện sinh của Sartre, tính thực tại đại diện cho những sự thật khách quan, cứng nhắc về quá khứ và giới hạn thể chất của chúng ta. Nó bao gồm năm sinh, chiều cao, gen di truyền và những lựa chọn chúng ta đã cam kết thực hiện trong quá khứ.",
              originalText:
                "Tính thực tại là bối cảnh định sẵn mà trong đó tự do của con người phải vận hành, đại diện cho thực tế chống lại tự do tuyệt đối. - Jean-Paul Sartre",
              quickTake:
                "Bối cảnh thực tại không thể thay đổi của sự tồn tại vật chất của đồng chí.",
              difficulty: "Medium",
              ...
              flashcards: [
                {
                  ...
                  answer:
                    "Tính thực tại là các sự thật khách quan trong cuộc đời một con người mà họ không thể thay đổi (như nguồn gốc, lịch sử, môi trường), đóng vai trò là chất liệu để tự do vận hành.",
                },
              ],
            },
          ],
          ...
          nodes: [
            {
              ...
              summary:
                "Sartre lập luận rằng con người tồn tại trước, sau đó chúng ta mới định nghĩa bản thân thông qua các lựa chọn và hành động của mình. Không có bản tính con người định sẵn, không có kế hoạch thần thánh nào cả, và không có lý do để bào chữa.",
              originalText:
                "Sự tồn tại đi trước bản chất. Con người bị kết án phải tự do; bởi vì một khi đã bị ném vào thế giới, anh ta phải chịu trách nhiệm về mọi hành động của mình. - Jean-Paul Sartre",
              quickTake:
                "Sự tồn tại đi trước bản chất. Chúng ta bị kết án phải tự do.",
              difficulty: "Hard",
              ...
              flashcards: [
                {
                  tag: "Thuyết Hiện sinh",
                  question:
                    "Cụm từ 'Sự tồn tại đi trước bản chất' nghĩa là gì?",
                  answer:
                    "Nó có nghĩa là con người không sinh ra với một mục đích định sẵn (bản chất). Chúng ta tồn tại trước, và phải tự định nghĩa ý nghĩa cuộc đời mình thông qua hành động và lựa chọn chủ động.",
                },
              ],
            },
          ],
