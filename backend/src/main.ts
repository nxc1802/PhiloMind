...
async function bootstrap() {
  ...
  // Setup Swagger Open API Docs in development or when explicitly enabled.
  const enableSwagger =
  ...
  if (enableSwagger) {
    const config = new DocumentBuilder()
      ...
      .setDescription(
        "Immersive AI philosophy learning endpoints including debate, podcasts, and spaced repetition flashcards.",
      )
      .setVersion("1.0.0")
      .addBearerAuth()
      .build();
  ...
  console.log(
    `[PhiloMind Backend] Service is running on port ${port} (bind 0.0.0.0)`,
  );
  if (enableSwagger) {
    console.log(
      `[PhiloMind Swagger] Documentation is available at http://localhost:${port}/docs`,
    );
  }
}
bootstrap();
