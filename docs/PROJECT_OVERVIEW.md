...
2. Journey and lesson loading:
   ...
   - Lesson access is blocked in the UI and learner API unless `contentReady === true` and `lessonStatus === "published"`.
   - The flow player renders `lessonFlow`, combines `lessonMedia` with legacy media components, and stores validated component progress through `/api/courses/nodes/:nodeId/component-progress`.
...
- Converted lesson component files under `data/lesson_components/` are the seed source for converted published lessons; `backend/prisma/reseed_lessons.ts` can refresh DB rows from the manifest.
